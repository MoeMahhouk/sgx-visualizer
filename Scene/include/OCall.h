    ...
    struct OCall : public Call{
        ...
        OCall(int id, int eid, uint64_t symbol_address, uint64_t start_time, uint64_t relative_start_time , uint64_t total_time,
              std::string symbol_name, uint64_t symbol_address_normalized, std::string symbol_file_name) :
                Call(id, eid, symbol_address,start_time,relative_start_time , total_time, symbol_name),
        ...
        virtual Rect* toRenderable(qreal factor) const override { //ToDo added factor for test purposes
            Rect *callBlock = new Rect(Transform2D(1,0,0,1,-2,relative_start_time_*factor),35,total_time_*factor,new QPen(Qt::red),new QBrush(Qt::yellow));
