    ...
    while(query.next())
    {
        ...
        switch(query.value(1).toInt()) { //EventType
            ...
            case (int)EventMap::EnclaveECallEvent: {
                ...
                symbol_address = (uint64_t)query.value(9).toDouble();
                symbol_name = query.value(10).toString().toStdString();
                bool is_private = query.value(11).toInt();
                ECall *eCall = new ECall(call_id,eid,symbol_address,start_time,relative_start_time,total_time,is_private,symbol_name,isFail);
                calls[id] = eCall;
            ...
            case (int)EventMap::EnclaveOCallEvent : {
                eid = query.value(12).toInt();
                symbol_name = query.value(13).toString().toStdString();
                std::string symbol_file_name = query.value(14).toString().toStdString();
                symbol_address = (uint64_t)query.value(15).toDouble();
                uint64_t symbol_address_normalized = (uint64_t)query.value(16).toDouble();
    ...
    if(!query.exec())
    {
        ...
        while (query.next())
        {
           ...
           // std::cerr << id << "   "  << symbol_name.toStdString() << std::endl;
            ...
            oCallTypeList.push_back(oCalltypeMember);
        }
    }
}
...
{
    QString availableThreads = " AND e1.involved_thread IN ( ";
    if (!threads_.isEmpty())
    {
