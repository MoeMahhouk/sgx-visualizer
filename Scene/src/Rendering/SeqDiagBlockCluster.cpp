...
{
    isClustered = false;
    //rect = new QGraphicsRectItem(transform.getX(), transform.getY(), width_ * transform.xScale(), height_ * transform.yScale());
}
...
{  //ToDo test it without the if, it might also work because we remove the item later from the scene when it hast height more than 150, so the ifs are redundant here
    if (rect->rect().height() < 150)  //ToDo only for test , if the cluster height is more than 150 pixel, it should draw the children eventually with their clusters
    {
        if(!isClustered)
        {
            //scene()->addItem(this);
